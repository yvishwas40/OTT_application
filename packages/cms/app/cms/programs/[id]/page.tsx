...
function ProgramDetailContent() {
  ...
  const handleQuickAddEpisode = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // 1. Get or Create Term (Season 1)
      let termId = '';
      if (program.terms && program.terms.length > 0) {
        termId = program.terms[0].id;
      } else {
        // Create Season 1
        const termRes = await api.post('/terms', {
          programId,
          termNumber: 1,
          title: 'Season 1'
        });
      ...
      // 2. Determine next lesson number
      const existingLessons = program.terms?.find((t: any) => t.id === termId)?.lessons || [];
      ...
      const lessonRes = await api.post('/lessons', {
        ...
        durationMs: newEpisodeData.durationMs ? parseInt(newEpisodeData.durationMs) * 60000 : null,
        isPaid: newEpisodeData.isPaid,
        contentLanguagePrimary: program.languagePrimary, // Default to program's primary
        contentLanguagesAvailable: [program.languagePrimary],
        contentUrlsByLanguage: {
          [program.languagePrimary]: newEpisodeData.contentUrl
        },
        subtitleLanguages: newEpisodeData.subtitleUrl ? [program.languagePrimary] : [],
        subtitleUrlsByLanguage: newEpisodeData.subtitleUrl ? {
          [program.languagePrimary]: newEpisodeData.subtitleUrl
        } : {},
        status: newEpisodeData.shouldPublish ? 'PUBLISHED' : 'DRAFT',
        publishedAt: newEpisodeData.shouldPublish ? new Date().toISOString() : null
      });

      // 4. Add Thumbnail (if provided)
      if (newEpisodeData.thumbnailUrl) {
        await api.post('/assets/lessons', {
          lessonId: lessonRes.data.id,
          language: program.languagePrimary,
          variant: 'LANDSCAPE', // Defaulting to Landscape for quick add
          assetType: 'thumbnail',
      ...
      fetchProgram();
      const modal = document.getElementById('add-episode-modal') as any;
      if (modal) modal.close();
    } catch (error: any) {
